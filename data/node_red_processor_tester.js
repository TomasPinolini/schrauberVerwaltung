/**
 * Node-RED Processor Tester
 * 
 * This script tests a specific processor with sample data from the all_data directory.
 ...
 * 
 * Example: 
 *   node node_red_processor_tester.js MOE6_Halle206_GH4
 *   node node_red_processor_tester.js MFV3_Halle204_Rest_CH MFV3_Halle204_Rest_CH.txt 5
 *   node node_red_processor_tester.js MOE6_Halle206_GH4 --all
 *   node node_red_processor_tester.js MOE6_Halle206_GH4 --count
 */
...
if (!controllerName) {
  ...
  console.error('Example: node node_red_processor_tester.js MOE6_Halle206_GH4');
...
try {
  ...
  if (sampleFile.endsWith('.json')) {
    ...
    if (options.processAll) {
      // Parse all lines for processing
      allSamples = lines.map((line, idx) => {
        try {
...
for (let i = 0; i < allSamples.length; i++) {
  ...
  try {
    ...
    // Check for errors first (new error handling structure)
    if (nodeErrors.length > 0 || result.error) {
      ...
      if (!options.processAll) {
        ...
        // Show Node-RED error logs
        if (nodeErrors.length > 0) {
          nodeErrors.forEach(err => console.error(`- ${err}`));
        }
        
        // Show error object if available
        if (result.error) {
