 ...
 * - Correctly decodes base64 data using 16-bit integers
 * 
 * Usage in Node-RED:
...
function decodeGraphB64(graph) {
  ...
  try {
    ...
    // Log the first few values for debugging (if available)
    if (angleValues.length > 0 && torqueValues.length > 0) {
      node.log(`First angle values: ${angleValues.slice(0, 3).join(', ')}`);
      node.log(`First torque values: ${torqueValues.slice(0, 3).join(', ')}`);
    }
